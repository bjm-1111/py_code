...
class FileErrorInfo(object):
    ...
    #错误信息集合
    ERROR_INFO = {
        FILE_NOT_EXIST: u"文件（{file_path}）不存在！",
        FILE_TYPE_ERROR: u"文件类型（只支持“{file_type}”）不支持！",
        GET_FILE_SIZE_FAIL: u"获取文件（{file_path}）大小失败！",
        READ_FILE_FAIL: u"读取文件（{file_path}）内容失败！",
        WRITE_FILE_FAIL: u"写入文件（{file_path}）失败",
    }